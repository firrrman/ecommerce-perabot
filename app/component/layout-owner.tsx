...
export default function LayoutOwner({
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("owner_read_orders");
  ...
  const markOrderAsRead = (orderId: string) => {
    setReadOrderIds((prev) => {
      ...
      try {
        localStorage.setItem("owner_read_orders", JSON.stringify(next));
      } catch (e) { }
      return next;
    });
  };

  const markAllOwnerOrdersAsRead = () => {
    const allIds = recentOrders.map((o) => o.id);
    setReadOrderIds(allIds);
    try {
      localStorage.setItem("owner_read_orders", JSON.stringify(allIds));
    } catch (e) { }
  };
  ...
  const menuItems = [
    ...
    { id: "products", name: "Produk", icon: Package, router: "/owner/produk" },
    {
      id: "orders",
      name: "Pesanan",
      icon: ShoppingCart,
      router: "/owner/pesanan",
    },
    {
      id: "customers",
      name: "Pelanggan",
      icon: Users,
      router: "/owner/pelanggan",
    },
    {
      id: "report",
      name: "Laporan",
      icon: TrendingUp,
      router: "/owner/laporan",
    },
  ];
        ...
        <div className="absolute bottom-0 w-full p-4 border-t border-gray-700 bg-gray-900">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium">Owner Perabotan</p>
